...
annotation.init = function () {
  ...
  document.addEventListener("keydown", (e) => {
    if (e.key === "a") {
      const annotateMode = deltaApp.state.annotateMode ? false : true;
      deltaApp.updateState({ annotateMode });
    }
  });

  deltaApp.eventDispatcher.on("stateChange:annotateMode", () => {
    annotation.toggleAnnotateMode();
  });

  annotation.canvasListeners();

  deltaApp.updateState({ drawing: false, annotateMode: false });
};

annotation.toggleAnnotateMode = function () {
  const canvasList = document.querySelectorAll("canvas");
  canvasList.forEach((canvas) => {
    canvas.style.pointerEvents = deltaApp.state.annotateMode
      ? "visible"
      : "none";
    canvas.style.cursor = deltaApp.state.annotateMode ? "crosshair" : "default";
  });
};
...
annotation.canvasListeners = function () {
  ...
  canvasList.forEach((canvas) => {
    ...
    canvas.addEventListener("mousedown", (e) => {
      ...
      if (deltaApp.state.annotateMode) {
        deltaApp.updateState({ drawing: true });
        ctx.beginPath();
      }
    });
    ...
    canvas.addEventListener("mousemove", (e) => {
      const ctx = e.target.getContext("2d");
      if (deltaApp.state.drawing && deltaApp.state.annotateMode) {
    ...
    canvas.addEventListener("mouseup", () => {
      deltaApp.updateState({ drawing: false });
    });
  });
};
...
deltaApp.eventDispatcher.on("deltaIsReady", annotation.init);
